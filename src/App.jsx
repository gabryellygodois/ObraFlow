...
export default function App() {
  ...
  useEffect(() => {
    ...
    const activeEmail = localStorage.getItem('obraflow_current_email');
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('obraflow_current_email');
    setIsLoggedIn(false);
    setPassword('');
    setEmail('');
    setAuthError('');
  ...
  const handleCreateProject = (e) => {
    ...
    const newProjectStructure = {
      ...
      category: activeTab,
      equipe: newProjEquipe.trim() || 'Apenas eu',
      diario: '',
      tasks: []
    };
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', backgroundColor: COLORS.bgLight, fontFamily: 'sans-serif' }}>
        <div style={{ width: '100%', maxWidth: '400px', padding: '32px', borderRadius: '16px', backgroundColor: COLORS.white, border: '1px solid #E5E7EB', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
          ...
          {authError && <div style={{ backgroundColor: '#FEE2E2', color: '#991B1B', padding: '10px', borderRadius: '8px', fontSize: '13px', textAlign: 'center', marginBottom: '16px', fontWeight: '500' }}>⚠️ {authError}</div>}

          <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {authMode === 'signup' && (
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', textTransform: 'uppercase', marginBottom: '6px', color: COLORS.slate }}>Tipo de Perfil</label>
                <select value={userRole} onChange={(e) => setUserRole(e.target.value)} style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', fontSize: '14px', border: '1px solid #D1D5DB', backgroundColor: COLORS.bgLight }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', textTransform: 'uppercase', marginBottom: '6px', color: COLORS.slate }}>E-mail corporativo</label>
          ...
          <div style={{ marginTop: '24px', textAlign: 'center', borderTop: '1px solid #E5E7EB', paddingTop: '16px' }}>
            <button onClick={() => { setAuthMode(authMode === 'login' ? 'signup' : 'login'); setAuthError(''); }} style={{ background: 'none', border: 'none', color: COLORS.oliveDark, fontWeight: '600', fontSize: '13px', cursor: 'pointer' }}>
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F3F4F6', fontFamily: 'sans-serif', color: COLORS.charcoal }}>
      <header style={{ backgroundColor: COLORS.white, padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #E5E7EB' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <span style={{ fontSize: '14px', fontWeight: '500', color: COLORS.slate }}>
            {userRole === 'engenheiro' ? `👷‍♂️ Eng. ${userName}` : `📐 Arq. ${userName}`}
          </span>
          <button onClick={() => setShowPasswordSection(!showPasswordSection)} style={{ padding: '6px 12px', fontSize: '12px', borderRadius: '8px', border: '1px solid #D1D5DB', backgroundColor: '#FFF', cursor: 'pointer', fontWeight: '600' }}>🔒 Mudar Senha</button>
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
              ...
              <div style={{ backgroundColor: '#FEF3C7', padding: '20px', borderRadius: '12px', border: '1px solid #FCD34D' }}>
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#B45309', textTransform: 'uppercase' }}>⏳ A Fazer</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
              ...
              <button onClick={() => setShowNewProjectForm(!showNewProjectForm)} style={{ padding: '10px 18px', backgroundColor: COLORS.charcoal, color: COLORS.white, border: 'none', borderRadius: '10px', fontWeight: '600', fontSize: '13px', cursor: 'pointer' }}>
                {showNewProjectForm ? '✕ Cancelar' : `+ Adicionar Nova Demanda`}
              </button>
            </div>

            {showNewProjectForm && (
              <div style={{ backgroundColor: COLORS.white, padding: '20px', borderRadius: '16px', border: `2px dashed ${COLORS.olive}`, marginBottom: '24px' }}>
                <h3 style={{ margin: '0 0 16px 0', fontSize: '15px' }}>Cadastrar Nova Demanda</h3>
                <form onSubmit={handleCreateProject} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    <input type="text" placeholder="Engenheiros / Profissionais vinculados" value={newProjEquipe} onChange={(e) => setNewProjEquipe(e.target.value)} style={{ flex: 1, minWidth: '200px', padding: '10px 14px', borderRadius: '8px', border: '1px solid #D1D5DB', fontSize: '14px' }} />
                  </div>
                  <button type="submit" style={{ alignSelf: 'flex-start', padding: '10px 20px', backgroundColor: COLORS.olive, color: COLORS.white, border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '13px', cursor: 'pointer' }}>Salvar Obra</button>
                </form>
              </div>
            )}
            ...
            <div style={{ backgroundColor: COLORS.white, padding: '24px', borderRadius: '16px', border: '1px solid #E5E7EB' }}>
              <div style={{ display: 'flex', borderBottom: '2px solid #E5E7EB', marginBottom: '20px', gap: '8px' }}>
                <button onClick={() => { setActiveTab('nre'); setShowNewProjectForm(false); }} style={{ padding: '12px 16px', fontSize: '14px', fontWeight: '600', border: 'none', background: 'none', cursor: 'pointer', borderBottom: activeTab === 'nre' ? `3px solid ${COLORS.olive}` : '3px solid transparent', color: activeTab === 'nre' ? COLORS.oliveDark : COLORS.ash }}>🏫 Demandas Públicas NRE</button>
                <button onClick={() => { setActiveTab('particulares'); setShowNewProjectForm(false); }} style={{ padding: '12px 16px', fontSize: '14px', fontWeight: '600', border: 'none', background: 'none', cursor: 'pointer', borderBottom: activeTab === 'particulares' ? `3px solid ${COLORS.charcoal}` : '3px solid transparent', color: activeTab === 'particulares' ? COLORS.charcoal : COLORS.ash }}>💼 Projetos Particulares</button>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {Object.keys(projects)
                  ...
                  .map(code => {
                    ...
                    return (
                      <div key={code} onClick={() => openProjectDetails(code)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderRadius: '12px', backgroundColor: '#F9FAFB', cursor: 'pointer', border: '1px solid #E5E7EB' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '11px', color: COLORS.oliveDark, marginTop: '4px' }}>👥 Equipe vinculada: {proj.equipe || 'Apenas eu'}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <span style={{ fontSize: '13px', fontWeight: 'bold', color: COLORS.oliveDark }}>{percentual}% Concluído</span>
                          <div style={{ fontSize: '11px', color: COLORS.ash }}>{finished} de {total} tarefas</div>
                        </div>
                      </div>
                    );
                  })}
          ...
          <>
            ...
            <div style={{ backgroundColor: COLORS.white, padding: '28px', borderRadius: '16px', border: '1px solid #E5E7EB' }}>
              <div style={{ borderBottom: '1px solid #E5E7EB', paddingBottom: '16px', marginBottom: '24px' }}>
                ...
                <p style={{ color: COLORS.oliveDark, margin: '6px 0 0 0', fontSize: '12px', fontWeight: '600' }}>👥 Profissionais na obra: {currentProject.equipe || 'Apenas eu'}</p>
              </div>
              ...
              <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #E5E7EB' }}>
                ...
                <textarea value={currentProject.diario} onChange={(e) => handleUpdateDiario(e.target.value)} placeholder="Insira aqui as anotações..." style={{ width: '100%', boxSizing: 'border-box', height: '110px', padding: '12px', borderRadius: '10px', border: '1px solid #D1D5DB', backgroundColor: COLORS.bgLight, fontSize: '13px', resize: 'vertical' }} />
              </div>
            </div>
          </>
        )}
